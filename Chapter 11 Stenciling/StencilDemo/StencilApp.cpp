...
void StencilApp::Draw(const GameTimer &gt) {
  ...
  // buffer is 1). Note that we must supply a different per-pass constant
  // buffer--one with the lights reflected.
  mCommandList->SetGraphicsRootConstantBufferView(
...
void StencilApp::OnMouseUp(WPARAM btnState, int x, int y) { ReleaseCapture(); }
...
void StencilApp::OnKeyboardInput(const GameTimer &gt) {
  ...
  XMMATRIX skullOffset = XMMatrixTranslation(
      mSkullTranslation.x, mSkullTranslation.y, mSkullTranslation.z);
...
void StencilApp::LoadTextures() {
  ...
  ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(
      md3dDevice.Get(), mCommandList.Get(), bricksTex->Filename.c_str(),
  ...
  ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(
      md3dDevice.Get(), mCommandList.Get(), checkboardTex->Filename.c_str(),
  ...
  ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(
      md3dDevice.Get(), mCommandList.Get(), iceTex->Filename.c_str(),
  ...
  ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(
      md3dDevice.Get(), mCommandList.Get(), white1x1Tex->Filename.c_str(),
...
void StencilApp::BuildDescriptorHeaps() {
  ...
  md3dDevice->CreateShaderResourceView(checkboardTex.Get(), &srvDesc,
                                       hDescriptor);
...
void StencilApp::BuildPSOs() {
  ...
  // We are going to draw shadows with transparency, so base it off the
...
void StencilApp::DrawRenderItems(ID3D12GraphicsCommandList *cmdList,
                                 const std::vector<RenderItem *> &ritems) {
  UINT objCBByteSize =
