...
struct ObjectConstants {
  ...
  XMFLOAT4 PulseColor = XMFLOAT4(Colors::Goldenrod);
  float Time = 0.0f;
};
...
void BoxApp::Draw(const GameTimer &gt) {
  ...
  auto name = "box";
  mCommandList->DrawIndexedInstanced(mBoxGeo->DrawArgs[name].IndexCount, 1,
                                     mBoxGeo->DrawArgs[name].StartIndexLocation,
                                     mBoxGeo->DrawArgs[name].BaseVertexLocation,
                                     0);
...
void BoxApp::BuildShadersAndInputLayout() {
  ...
  mInputLayout = {{"COLOR", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 12,
...
void BoxApp::BuildBoxGeometry() {
  ...
  std::array<VColorData, 13> vcolor = {
      VColorData{XMCOLOR(Colors::White)}, VColorData{XMCOLOR(Colors::Black)},
      VColorData{XMCOLOR(Colors::Red)},   VColorData{XMCOLOR(Colors::Green)},
      VColorData{XMCOLOR(Colors::Blue)},  VColorData{XMCOLOR(Colors::Yellow)},
      VColorData{XMCOLOR(Colors::Cyan)},  VColorData{XMCOLOR(Colors::Magenta)},

      VColorData{XMCOLOR(Colors::Red)},   VColorData{XMCOLOR(Colors::Green)},
      VColorData{XMCOLOR(Colors::Green)}, VColorData{XMCOLOR(Colors::Green)},
  ...
  const UINT vpbByteSize = (UINT)vpos.size() * sizeof(VPosData);
  const UINT vcbByteSize = (UINT)vcolor.size() * sizeof(VColorData);
  ...
  CopyMemory(mBoxGeo->VPosBufferCPU->GetBufferPointer(), vpos.data(),
             vpbByteSize);
  mBoxGeo->VPosBufferGPU = d3dUtil::CreateDefaultBuffer(
      md3dDevice.Get(), mCommandList.Get(), vpos.data(), vpbByteSize,
      mBoxGeo->VPosBufferUploader);

  ThrowIfFailed(D3DCreateBlob(vcbByteSize, &mBoxGeo->VColorBufferCPU));
  CopyMemory(mBoxGeo->VColorBufferCPU->GetBufferPointer(), vcolor.data(),
             vcbByteSize);
  mBoxGeo->VColorBufferGPU = d3dUtil::CreateDefaultBuffer(
      md3dDevice.Get(), mCommandList.Get(), vcolor.data(), vcbByteSize,
  ...
  SubmeshGeometry triangle;
  triangle.IndexCount = 18;
  triangle.StartIndexLocation = 36;
  triangle.BaseVertexLocation = 8;
  mBoxGeo->DrawArgs["tri"] = triangle;
}
