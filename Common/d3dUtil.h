...
struct MeshGeometry {
  ...
  D3D12_VERTEX_BUFFER_VIEW VPosBufferView() const {
    D3D12_VERTEX_BUFFER_VIEW vpv;
    vpv.BufferLocation = VPosBufferGPU->GetGPUVirtualAddress();
    vpv.StrideInBytes = VPosByteStride;
    vpv.SizeInBytes = VPosBufferByteSize;

    return vpv;
  }

  D3D12_VERTEX_BUFFER_VIEW VColorBufferView() const {
    D3D12_VERTEX_BUFFER_VIEW vcv;
    vcv.BufferLocation = VColorBufferGPU->GetGPUVirtualAddress();
    vcv.StrideInBytes = VColorByteStride;
    vcv.SizeInBytes = VColorBufferByteSize;
    return vcv;
  }
