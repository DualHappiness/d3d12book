...
private:
  ...
  void DrawRenderItemsNormal(ID3D12GraphicsCommandList *cmdList,
...
void BlendApp::Draw(const GameTimer &gt) {
  ...
  mCommandList->ClearRenderTargetView(
      CurrentBackBufferView(), (float *)&mMainPassCB.FogColor, 0, nullptr);
  ...
  mCommandList->SetPipelineState(mPSOs["normalVisualize"].Get());
  DrawRenderItemsNormal(mCommandList.Get(), mAllRitems);
  // Indicate a state transition on the resource usage.
  mCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
                                       CurrentBackBuffer(),
                                       D3D12_RESOURCE_STATE_RENDER_TARGET,
...
void BlendApp::OnMouseUp(WPARAM btnState, int x, int y) { ReleaseCapture(); }
...
void BlendApp::LoadTextures() {
  ...
  ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(
      md3dDevice.Get(), mCommandList.Get(), grassTex->Filename.c_str(),
  ...
  ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(
      md3dDevice.Get(), mCommandList.Get(), waterTex->Filename.c_str(),
  ...
  ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(
      md3dDevice.Get(), mCommandList.Get(), fenceTex->Filename.c_str(),
...
void BlendApp::BuildShadersAndInputLayout() {
  ...
  const D3D_SHADER_MACRO alphaTestDefines[] = {"FOG", "1",  "ALPHA_TEST",
                                               "1",   NULL, NULL};
  ...
  mShaders["alphaTestedPS"] = d3dUtil::CompileShader(
      L"Shaders\\Default.hlsl", alphaTestDefines, "PS", "ps_5_0");

  mShaders["normalVisualizeVS"] = d3dUtil::CompileShader(
      L"Shaders\\NormalVisualize.hlsl", nullptr, "VS", "vs_5_0");
  mShaders["normalVisualizeGS"] = d3dUtil::CompileShader(
      L"Shaders\\NormalVisualize.hlsl", nullptr, "GS", "gs_5_0");
  mShaders["normalVisualizePS"] = d3dUtil::CompileShader(
      L"Shaders\\NormalVisualize.hlsl", defines, "PS", "ps_5_0");
...
void BlendApp::BuildCylinderGeometry() {
  ...
  GeometryGenerator::MeshData cylinder =
      geoGen.CreateCylinder(8.0f, 8.0f, 16.0f, 8, 8);
...
void BlendApp::BuildPSOs() {
  ...
  depthTestBlendDesc.SrcBlendAlpha = D3D12_BLEND_ZERO;
  ...
  // opaquePsoDesc.BlendState.RenderTarget[0] = depthTestBlendDesc;
  ...
  alphaTestedBlendDesc.DestBlend = D3D12_BLEND_ZERO;
  ...
  alphaTestedBlendDesc.RenderTargetWriteMask =
      D3D12_COLOR_WRITE_ENABLE_ALPHA - 1;

  // alphaTestedPsoDesc.BlendState.RenderTarget[0] = alphaTestedBlendDesc;
  // alphaTestedPsoDesc.DepthStencilState.DepthWriteMask =
  // D3D12_DEPTH_WRITE_MASK_ZERO;
  ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(
      &alphaTestedPsoDesc, IID_PPV_ARGS(&mPSOs["alphaTested"])));

  D3D12_GRAPHICS_PIPELINE_STATE_DESC normalVisualizePsoDesc = opaquePsoDesc;
  normalVisualizePsoDesc.VS = {
      reinterpret_cast<BYTE *>(
          mShaders["normalVisualizeVS"]->GetBufferPointer()),
      mShaders["normalVisualizeVS"]->GetBufferSize()};
  normalVisualizePsoDesc.GS = {
      reinterpret_cast<BYTE *>(
          mShaders["normalVisualizeGS"]->GetBufferPointer()),
      mShaders["normalVisualizeGS"]->GetBufferSize()};
  normalVisualizePsoDesc.PS = {
      reinterpret_cast<BYTE *>(
          mShaders["normalVisualizePS"]->GetBufferPointer()),
      mShaders["normalVisualizePS"]->GetBufferSize()};

  ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(
      &normalVisualizePsoDesc, IID_PPV_ARGS(&mPSOs["normalVisualize"])));
}
...
void BlendApp::BuildMaterials() {
  ...
  // This is not a good water material definition, but we do not have all the
  // rendering tools we need (transparency, environment reflection), so we fake
  // it for now.
...
void BlendApp::BuildRenderItems() {
  ...
  XMStoreFloat4x4(&cylinderRitem->World,
                  XMMatrixTranslation(3.0f, 8.0f, -9.0f));
  ...
  cylinderRitem->IndexCount =
      cylinderRitem->Geo->DrawArgs["cylinder"].IndexCount;
  cylinderRitem->StartIndexLocation =
      cylinderRitem->Geo->DrawArgs["cylinder"].StartIndexLocation;
  cylinderRitem->BaseVertexLocation =
      cylinderRitem->Geo->DrawArgs["cylinder"].BaseVertexLocation;
...
void BlendApp::DrawRenderItemsNormal(ID3D12GraphicsCommandList *cmdList,
                               const std::vector<std::unique_ptr<RenderItem>> &ritems) {
  UINT objCBByteSize =
      d3dUtil::CalcConstantBufferByteSize(sizeof(ObjectConstants));
  UINT matCBByteSize =
