...
void VecAddCSApp::OnMouseUp(WPARAM btnState, int x, int y) { ReleaseCapture(); }
...
void VecAddCSApp::DoComputeWork() {
  ...
  // via ExecuteCommandList. Reusing the command list reuses memory.
  ThrowIfFailed(
      mCommandList->Reset(mDirectCmdListAlloc.Get(), mPSOs["vecLen"].Get()));
  ...
  mCommandList->SetComputeRootUnorderedAccessView(
      1, mOutputBuffer->GetGPUVirtualAddress());
  mCommandList->SetComputeRootUnorderedAccessView(
      0, mInputBufferA->GetGPUVirtualAddress());
...
void VecAddCSApp::BuildBuffers() {
  ...
  std::vector<Data2> data(NumDataElements);
  for (int i = 0; i < NumDataElements; ++i) {
    data[i].vec = XMFLOAT3(1, 1, 1);
  }

  // Create some buffers to be used as SRVs.
  auto byteSize = (UINT)data.size() * sizeof(Data2);
  ...
  mCommandList->ResourceBarrier(
      1, &CD3DX12_RESOURCE_BARRIER::Transition(mInputBufferA.Get(),
                                               D3D12_RESOURCE_STATE_COMMON,
                                               D3D12_RESOURCE_STATE_COPY_DEST));
  UpdateSubresources<1>(mCommandList.Get(), mInputBufferA.Get(), mInputUploadBufferA.Get(), 0, 0,
                        1, &subResourceData);
  mCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
                                  mInputBufferA.Get(),
                                  D3D12_RESOURCE_STATE_COPY_DEST,
  ...
  ThrowIfFailed(md3dDevice->CreateCommittedResource(
      ...
      &CD3DX12_RESOURCE_DESC::Buffer((UINT)data.size() * sizeof(float)),
...
void VecAddCSApp::BuildShadersAndInputLayout() {
  mShaders["vecAddCS"] =
      d3dUtil::CompileShader(L"Shaders\\VecAdd.hlsl", nullptr, "CS", "cs_5_0");
  mShaders["vecLenCS"] =
      d3dUtil::CompileShader(L"Shaders\\VecLen.hlsl", nullptr, "CS", "cs_5_0");
}

void VecAddCSApp::BuildPSOs() {
  D3D12_COMPUTE_PIPELINE_STATE_DESC computePsoDesc = {};
  auto size = mShaders["vecLenCS"]->GetBufferSize();
  ...
  ThrowIfFailed(md3dDevice->CreateComputePipelineState(
      &computePsoDesc, IID_PPV_ARGS(&mPSOs["vecLen"])));
}

void VecAddCSApp::BuildFrameResources() {
  for (int i = 0; i < gNumFrameResources; ++i) {
    mFrameResources.push_back(
        std::make_unique<FrameResource>(md3dDevice.Get(), 1));
  }
}

std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6>
VecAddCSApp::GetStaticSamplers() {
