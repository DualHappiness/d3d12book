...
BlurFilter::BlurFilter(ID3D12Device *device, UINT width, UINT height,
                       DXGI_FORMAT format) {
  md3dDevice = device;
...
ID3D12Resource *BlurFilter::Output() { return mBlurMap0.Get(); }
...
void BlurFilter::Execute(ID3D12GraphicsCommandList *cmdList,
  ...
  cmdList->SetComputeRoot32BitConstants(0, (UINT)weights.size(), weights.data(),
                                        1);
  ...
  cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
                                  mBlurMap1.Get(), D3D12_RESOURCE_STATE_COMMON,
  ...
  cmdList->ResourceBarrier(1,
                           &CD3DX12_RESOURCE_BARRIER::Transition(
                               mBlurMap1.Get(), D3D12_RESOURCE_STATE_COPY_DEST,
                               D3D12_RESOURCE_STATE_GENERIC_READ));

  cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
                                  mBlurMap0.Get(), D3D12_RESOURCE_STATE_COMMON,
                                  D3D12_RESOURCE_STATE_UNORDERED_ACCESS));

  cmdList->SetPipelineState(horzBlurPSO);
  cmdList->SetComputeRootDescriptorTable(2, mBlur0GpuSrv);
  cmdList->SetComputeRootDescriptorTable(1, mBlur1GpuUav);

  // How many groups do we need to dispatch to cover a row of pixels,
  // where each group covers 256 pixels (the 256 is defined in the
  // ComputeShader).
  UINT numGroupsX = (UINT)ceilf(mWidth / 16.0f);
  cmdList->Dispatch(numGroupsX, (UINT)ceilf(mHeight / 16.0f), 1);
...
void BlurFilter::BuildDescriptors() {
  ...
  md3dDevice->CreateUnorderedAccessView(mBlurMap0.Get(), nullptr, &uavDesc,
                                        mBlur0CpuUav);

  md3dDevice->CreateShaderResourceView(mBlurMap1.Get(), &srvDesc, mBlur1CpuSrv);
  md3dDevice->CreateUnorderedAccessView(mBlurMap1.Get(), nullptr, &uavDesc,
                                        mBlur1CpuUav);
}
