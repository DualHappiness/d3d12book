...
bool BlurApp::Initialize() {
  ...
  mBlurFilter =
      std::make_unique<BlurFilter>(md3dDevice.Get(), mClientWidth,
...
void BlurApp::OnMouseUp(WPARAM btnState, int x, int y) { ReleaseCapture(); }
...
void BlurApp::BuildDescriptorHeaps() {
  ...
  ThrowIfFailed(md3dDevice->CreateDescriptorHeap(
      &srvHeapDesc, IID_PPV_ARGS(&mCbvSrvUavDescriptorHeap)));
  ...
  mBlurFilter->BuildDescriptors(
      CD3DX12_CPU_DESCRIPTOR_HANDLE(
          mCbvSrvUavDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), 3,
          mCbvSrvUavDescriptorSize),
      CD3DX12_GPU_DESCRIPTOR_HANDLE(
          mCbvSrvUavDescriptorHeap->GetGPUDescriptorHandleForHeapStart(), 3,
          mCbvSrvUavDescriptorSize),
      mCbvSrvUavDescriptorSize);
}
...
void BlurApp::BuildShadersAndInputLayout() {
  ...
  mShaders["horzBlurCS"] = d3dUtil::CompileShader(
      L"Shaders\\Blur.hlsl", nullptr, "HorzBlurCS", "cs_5_0");
  mShaders["vertBlurCS"] = d3dUtil::CompileShader(
      L"Shaders\\Blur.hlsl", nullptr, "VertBlurCS", "cs_5_0");
  mShaders["bilertalBlurCS"] = d3dUtil::CompileShader(
      L"Shaders\\Blur.hlsl", nullptr, "BilateralCS", "cs_5_0");
...
void BlurApp::BuildPSOs() {
  ...
  horzBlurPSO.CS = {
      reinterpret_cast<BYTE *>(mShaders["bilertalBlurCS"]->GetBufferPointer()),
      mShaders["bilertalBlurCS"]->GetBufferSize()};
  horzBlurPSO.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
  ThrowIfFailed(md3dDevice->CreateComputePipelineState(
      &horzBlurPSO, IID_PPV_ARGS(&mPSOs["horzBlur"])));
  ...
  ThrowIfFailed(md3dDevice->CreateComputePipelineState(
      &vertBlurPSO, IID_PPV_ARGS(&mPSOs["vertBlur"])));
}
...
void BlurApp::DrawRenderItems(ID3D12GraphicsCommandList *cmdList,
                              const std::vector<RenderItem *> &ritems) {
  UINT objCBByteSize =
