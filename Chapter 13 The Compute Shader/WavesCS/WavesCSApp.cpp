...
private:
  ...
  void BuildPostProcessRootSignatrue();
...
bool WavesCSApp::Initialize() {
  ...
  mWaves = std::make_unique<GpuWaves>(md3dDevice.Get(), mCommandList.Get(), 256,
                                      256, 0.25f, 0.03f, 2.0f, 0.2f);
...
void WavesCSApp::Draw(const GameTimer &gt) {
  ...
  mCommandList->ClearRenderTargetView(
      mOffscreenRT->Rtv(), (float *)&mMainPassCB.FogColor, 0, nullptr);
  mCommandList->ClearDepthStencilView(
      DepthStencilView(), D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL,
      1.0f, 0, 0, nullptr);

  // Specify the buffers we are going to render to.
  mCommandList->OMSetRenderTargets(1, &mOffscreenRT->Rtv(), true,
                                   &DepthStencilView());
...
void WavesCSApp::OnMouseUp(WPARAM btnState, int x, int y) { ReleaseCapture(); }
...
void WavesCSApp::UpdateWavesGPU(const GameTimer &gt) {
  ...
  mWaves->Update(gt, mCommandList.Get(), mWavesRootSignature.Get(),
...
void WavesCSApp::BuildRootSignature() {
  ...
  CD3DX12_ROOT_SIGNATURE_DESC rootSigDesc(
      5, slotRootParameter, (UINT)staticSamplers.size(), staticSamplers.data(),
...
void WavesCSApp::BuildPostProcessRootSignatrue() {
...
void WavesCSApp::BuildDescriptorHeaps() {
  ...
  mWaves->BuildDescriptors(
      CD3DX12_CPU_DESCRIPTOR_HANDLE(srvCpuStart, waveSrvOffset,
                                    mCbvSrvDescriptorSize),
      CD3DX12_GPU_DESCRIPTOR_HANDLE(srvGpuStart, waveSrvOffset,
                                    mCbvSrvDescriptorSize),
      mCbvSrvDescriptorSize);

  mSobelFilter->BuildDescriptors(
      CD3DX12_CPU_DESCRIPTOR_HANDLE(srvCpuStart, sobelSrvOffset,
                                    mCbvSrvDescriptorSize),
      CD3DX12_GPU_DESCRIPTOR_HANDLE(srvGpuStart, sobelSrvOffset,
                                    mCbvSrvDescriptorSize),
      mCbvSrvDescriptorSize);

  mOffscreenRT->BuildDescriptors(
      CD3DX12_CPU_DESCRIPTOR_HANDLE(srvCpuStart, offscreenSrvOffset,
                                    mCbvSrvDescriptorSize),
      CD3DX12_GPU_DESCRIPTOR_HANDLE(srvGpuStart, offscreenSrvOffset,
                                    mCbvSrvDescriptorSize),
      CD3DX12_CPU_DESCRIPTOR_HANDLE(rtvCpuStart, rtvOffset,
                                    mRtvDescriptorSize));
}
...
void WavesCSApp::BuildShadersAndInputLayout() {
  ...
  mShaders["wavesVS"] = d3dUtil::CompileShader(L"Shaders\\Default.hlsl",
                                               waveDefines, "VS", "vs_5_0");
  mShaders["opaquePS"] =
  ...
  mShaders["wavesDisturbCS"] = d3dUtil::CompileShader(
      L"Shaders\\WaveSim.hlsl", nullptr, "DisturbWavesCS", "cs_5_0");
  mShaders["sobelCS"] = d3dUtil::CompileShader(L"Shaders\\Sobel.hlsl", nullptr,
                                               "SobelCS", "cs_5_0");
  mShaders["compositeVS"] = d3dUtil::CompileShader(L"Shaders\\Composite.hlsl",
                                                   nullptr, "VS", "vs_5_0");
  mShaders["compositePS"] = d3dUtil::CompileShader(L"Shaders\\Composite.hlsl",
                                                   nullptr, "PS", "ps_5_0");
...
void WavesCSApp::BuildWavesGeometry() {
  ...
  GeometryGenerator::MeshData grid = geoGen.CreateGrid(
      160.0f, 160.0f, mWaves->RowCount(), mWaves->ColumnCount());
...
void WavesCSApp::BuildPSOs() {
  ...
  ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(
      &wavesRenderPSO, IID_PPV_ARGS(&mPSOs["wavesRender"])));
  ...
  ThrowIfFailed(md3dDevice->CreateComputePipelineState(
      &wavesDisturbPSO, IID_PPV_ARGS(&mPSOs["wavesDisturb"])));
  ...
  ThrowIfFailed(md3dDevice->CreateComputePipelineState(
      &sobelPSO, IID_PPV_ARGS(&mPSOs["sobel"])));
  ...
  ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(
      &compositePSO, IID_PPV_ARGS(&mPSOs["composite"])));
}
...
void WavesCSApp::DrawRenderItems(ID3D12GraphicsCommandList *cmdList,
                                 const std::vector<RenderItem *> &ritems) {
  UINT objCBByteSize =
...
void WavesCSApp::DrawFullscreenQuad(ID3D12GraphicsCommandList *cmdList) {
