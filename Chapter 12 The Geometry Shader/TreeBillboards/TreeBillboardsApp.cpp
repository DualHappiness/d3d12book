...
private:
  ...
  void DrawTrees(ID3D12GraphicsCommandList *cmdList,
                       const std::vector<RenderItem *> &ritems);
...
void TreeBillboardsApp::Draw(const GameTimer &gt) {
  ...
  DrawTrees(mCommandList.Get(),
                  mRitemLayer[(int)RenderLayer::AlphaTestedTreeSprites]);
...
void TreeBillboardsApp::LoadTextures() {
  ...
  ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(
      md3dDevice.Get(), mCommandList.Get(), treeArrayTex->Filename.c_str(),
...
void TreeBillboardsApp::BuildDescriptorHeaps() {
  ...
  md3dDevice->CreateShaderResourceView(treeArrayTex.Get(), &srvDesc,
                                       hDescriptor);
}
...
void TreeBillboardsApp::BuildShadersAndInputLayouts() {
  ...
  mShaders["standardVS"] =
      d3dUtil::CompileShader(L"Shaders\\Default.hlsl", nullptr, "GeoVS", "vs_5_0");
  mShaders["sphereGS"] =
      d3dUtil::CompileShader(L"Shaders\\Default.hlsl", nullptr, "GS", "gs_5_0");
  mShaders["opaquePS"] =
  ...
  mShaders["treeSpriteVS"] = d3dUtil::CompileShader(L"Shaders\\TreeSprite.hlsl",
                                                    nullptr, "VS", "vs_5_0");
  mShaders["treeSpriteGS"] = d3dUtil::CompileShader(L"Shaders\\TreeSprite.hlsl",
                                                    nullptr, "GS", "gs_5_0");
  mShaders["treeSpritePS"] = d3dUtil::CompileShader(
      L"Shaders\\TreeSprite.hlsl", alphaTestDefines, "PS", "ps_5_0");
...
void TreeBillboardsApp::BuildTreeSpritesGeometry() {
  ...
  std::array<std::uint16_t, 16> indices = {0, 1, 2,  3,  4,  5,  6,  7,
...
void TreeBillboardsApp::BuildPSOs() {
  ...
  opaquePsoDesc.InputLayout = {mStdInputLayout.data(),
                               (UINT)mStdInputLayout.size()};
...
void TreeBillboardsApp::BuildRenderItems() {
  ...
  XMStoreFloat4x4(&boxRitem->World, XMMatrixTranslation(4, 4, -9));
  sphereRitem->ObjCBIndex = 2;
  sphereRitem->Mat = mMaterials["wirefence"].get();
  sphereRitem->Geo = mGeometries["sphereGeo"].get();
  sphereRitem->PrimitiveType = D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
  sphereRitem->IndexCount = sphereRitem->Geo->DrawArgs["geoSphere"].IndexCount;
  sphereRitem->StartIndexLocation =
      sphereRitem->Geo->DrawArgs["geoSphere"].StartIndexLocation;
  sphereRitem->BaseVertexLocation =
      sphereRitem->Geo->DrawArgs["geoSphere"].BaseVertexLocation;
  ...
  mRitemLayer[(int)RenderLayer::AlphaTestedTreeSprites].push_back(
...
void TreeBillboardsApp::DrawTrees(
    ID3D12GraphicsCommandList *cmdList,
    const std::vector<RenderItem *> &ritems) {
  UINT objCBByteSize =
      d3dUtil::CalcConstantBufferByteSize(sizeof(ObjectConstants));
...
TreeBillboardsApp::GetStaticSamplers() {
